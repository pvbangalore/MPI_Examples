/* Sample program to illusrate how to distribute an array
   using nonblocking point-to-point communication primitives.

   Note that this version posts the nonblocking receive
...
int main(int argc, char **argv) {
  ...
  MPI_Irecv(&myimage[0][0], myM*N, MPI_INT, 0, 0, MPI_COMM_WORLD, &req[size]);
  if (rank == 0) {
  ...
  MPI_Wait(&req[size], MPI_STATUS_IGNORE);
