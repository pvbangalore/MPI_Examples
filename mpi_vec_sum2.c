 ...
 *
 * This version uses MPI_Scatter function to distribute the data 
 * that is initialized in the process with rank 0. 
 * This program also illustrate the use of MPI_Allreduce function.
 *
 * To Compile: mpicc -O -Wall -o mpi_vec_sum2 mpi_vec_sum2.c 
 * To Run: mpiexec -n 4 ./mpi_vec_sum2 100000 (on your local system)
 *         To run on ASA cluster follow instructions in README.
 */
